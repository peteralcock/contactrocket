...
module ContactRocketCRM
  module Callback
    @@classes   = []  # Classes that inherit from ContactRocketCRM::Callback::Base.
    ...
    # Adds a class inherited from from ContactRocketCRM::Callback::Base.
    #--------------------------------------------------------------------------
    def self.add(klass)
      @@classes << klass
    end
    ...
    class Base
      ...
      def self.inherited(child)
        ContactRocketCRM::Callback.add(child)
    ...
    # This makes it possible to call hook() without ContactRocketCRM::Callback prefix.
    ...
    module Helper
      def hook(method, caller, context = {}, &block)
        ...
        if is_view_hook
          hooks = ContactRocketCRM::Callback.view_hook(method, caller, context)
          ...
          if hooks[:replace].empty?
            view_data << if block_given?
                           ...
                           # legacy view hooks
                           ContactRocketCRM::Callback.hook(method, caller, context)
              end
          else
...
end # module ContactRocketCRM

ActionView::Base.send(:include, ContactRocketCRM::Callback::Helper)
ActionController::Base.send(:include, ContactRocketCRM::Callback::Helper)
