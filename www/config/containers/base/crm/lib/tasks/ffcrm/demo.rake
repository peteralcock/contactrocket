...
namespace :crcrm do
  namespace :demo do
    ...
    task load: :environment do
      ...
      Dir.glob(ContactRocketCRM.root.join('db', 'demo', '*.{yml,csv}')).each do |fixture_file|
        ActiveRecord::Fixtures.create_fixtures(ContactRocketCRM.root.join('db/demo'), File.basename(fixture_file, '.*'))
      end
    ...
    task reload: :environment do
      ...
      Rake::Task["crcrm:demo:load"].invoke
    end
  end
end
