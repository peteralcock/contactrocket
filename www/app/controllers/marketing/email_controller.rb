class EmailController < ApplicationController
  ...
  def test_delivery(action, redirect, async_success_notice, sync_success_notice)
    ...
    # if GlobalSettings.asynchronous? && action != MarketingRocketMailer::PREVIEW
      if true && action != MarketingRocketMailer::PREVIEW
      ...
      begin
        MarketingRocketMailer.phish(campaign.id, campaign.test_victim.email_address, blast.id, action)
  ...
  def preview
    test_delivery(MarketingRocketMailer::PREVIEW,
                  "/letter_opener",
  ...
  def test
    test_delivery(MarketingRocketMailer::ACTIVE,
                  :back,
  ...
  def launch
    ...
    #     begin
    #       MarketingRocketMailer.phish(campaign.id, target.email_address, blast.id, MarketingRocketMailer::ACTIVE)
    #     rescue ::NoMethodError
    #       flash[:error] = "Template is missing an email file, upload and create new email"
    #       break
    #     rescue Exception => e
